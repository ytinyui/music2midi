...
class PopDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.dataset.midi_notes = [truncate_notes(notes) for notes in self.dataset.midi_notes]
...
class PopDataset(Dataset):
    def __init__(self, data_dir: str, config_path:str="config.yaml"):
        ...
        self.beattimes = [np.load((self.data_dir / song_id).glob("*beattime.npy").__next__()) for song_id in song_ids]
        # self.beatsteps = [np.load((self.data_dir / song_id).glob("*beatstep.npy").__next__()) for song_id in song_ids]
    ...
    def __getitem__(self, index) -> (torch.Tensor, torch.Tensor):
        ...
        while True:
            ...
            notes_segment = get_notes_segment(self.midi_notes[index], start_idx, start_idx + n_bars * 8)
        ...
        tokens = self.tokenizer.notes_to_relative_tokens(notes_segment, offset_idx=start_idx, add_eos=True)
...
def get_audio_segment(audio_path:Path,
...
def get_notes_segment(notes:np.ndarray, start_idx:int, end_idx:int) -> np.ndarray:
    notes_segment = list(filter(lambda x: start_idx <= x[0] < end_idx, notes))
